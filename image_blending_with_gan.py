...
from model import RealismCNN, total_variation
...
def color_adjust(x, args):
    ...
    x_vgg_var = Variable(chainer.dataset.concat_examples([x_vgg*args.mask+args.bg_vgg*args.inverse_mask], args.gpu))

    # Poisson loss
    poisson_loss = F.sum(F.square((args.content_laplace - F.convolution_2d(x_vgg_var, W=args.W_laplace, pad=1))*args.mask_var))
    ...
    loss = args.poisson_weight * poisson_loss + args.tv_weight * tv_loss + args.realism_weight * realism_loss 
    
    # Backward
    loss.backward()
    # Transfer loss & diff from GPU to CPU
    loss = cuda.to_cpu(loss.data)
    dx = np.squeeze(cuda.to_cpu(x_vgg_var.grad)) * args.mask
...
def main():
    ...
    parser.add_argument('--realism_weight', type=float, default=1e1, help='Weight for realism loss')
    parser.add_argument('--tv_weight', type=float, default=5e-3, help='Weight for tv loss')
    ...
    parser.add_argument('--result_folder', default='transient_attributes_result/mask', help='Name for folder storing results')
    ...
    args.prefix_name = '_'.join(sorted(['{}({})'.format(key, value) for key, value in vars(args).items()
        if key in set(['poisson_weight', 'realism_weight', 'tv_weight', 'n_iteration', 'rand_init'])]))
    ...
    for i in range(args.total_instance):
        ...
        ## Laplace
        content_laplace = F.convolution_2d(obj_var, W=args.W_laplace, pad=1)
